...
import type { SessionIntegrity } from "../schemas/session.schema.js";
 ...
 */
export type ChainVerdictStatus = "verified" | "unchained" | "empty" | "incomplete" | "tampered";
...
// Three-state view of `session.yaml` as seen by the verifier.
type AnchorState =
  | { kind: "absent" }
  | { kind: "unreadable" }
  | { kind: "present"; integrity: SessionIntegrity | undefined };
...
export async function verifyEventsChain(
  ...
  // A chained file must end in a terminating newline; an unterminated tail
  // can only come from out-of-band editing (imports are atomic bulk writes).
