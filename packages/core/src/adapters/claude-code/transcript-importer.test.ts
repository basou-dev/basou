...
describe("claudeTranscriptToImportPayload with git-observed files", () => {
  ...
  it("records a file the transcript could never name", () => {
  ...
  it("emits NO event for an observation, so a re-import cannot duplicate one", () => {
    const payload = claudeTranscriptToImportPayload(shellOnly, {
      workspaceId: WS_ID,
      observedFiles: [
        { path: `${CWD}/a.ts`, change_type: "added" },
        { path: `${CWD}/gone.ts`, change_type: "deleted" },
      ],
    });
    if (payload === null) throw new Error("expected a payload");
    // An observation is a snapshot each pass recomputes; the event stream is
    // append-only and a re-import preserves every event it did not derive.
    expect(payload.events.filter((e) => e.type === "file_changed")).toEqual([]);
    expect(payload.session.related_files).toEqual([`${CWD}/a.ts`, `${CWD}/gone.ts`]);
  });

  it("lists a file a tool call already named exactly once", () => {
    const records: ClaudeTranscriptRecord[] = [
      { type: "user", timestamp: "2026-09-22T00:00:00.000Z", cwd: CWD, message: { content: [] } },
      {
        type: "assistant",
    ...
    if (payload === null) throw new Error("expected a payload");
    expect(payload.session.related_files).toEqual([`${CWD}/a.ts`]);
    // The tool call keeps its event; the observation adds no second one.
    expect(payload.events.filter((e) => e.type === "file_changed")).toHaveLength(1);
    expect(payload.events.find((e) => e.type === "file_changed")?.source).toBe(
      CLAUDE_IMPORT_SOURCE,
    );
  });
