 ...
 *
 * Core is stamped separately because it is a separate artifact: the CLI
 * does not bundle it, so `cli/dist` and `core/dist` can be rebuilt
 * independently, and a fresh CLI in front of a stale core is the dangerous
 * half of that pair.
 *
