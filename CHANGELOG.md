...
- **The timestamp gate is the expression itself, and every boundary it draws is
  ...
  Only one boundary had been pinned before -- that seconds are optional -- and
  the rest were implied by three examples. Reverting the offset requirement, or
  admitting lowercase designators, passed the whole suite: the drift guard
  compares generated bytes against committed bytes, so a source edit moves both
  sides together and the guard stays green. Tests now hold each boundary by
  name, in the schema and again in the published artifact: seconds optional, an
  offset or `Z` required, uppercase designators only, no leap second, and only
  calendar days that exist, century rule included.
