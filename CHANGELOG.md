...
- **A session's observed file changes no longer include commits it only
  ...
  whatever `diff.renames` says. Files dirty at the start are now read the same
  way the session's changes are, so a file with a conflict at the start, both
  names of a staged rename, and a name with leading or trailing spaces stay
  out of the session's files. When HEAD does not resolve to a commit, or moved
  while no reflog recorded anything, nothing is filtered; when git fails while
  working out the limit, the files observed last are kept.
  ...
  a namespace. Which reflog entries are recognised, and those limits, follow
  git's own behaviour and may change within a line; the claim does not. A
  session observed before the upgrade keeps the files its last observation
  recorded, re-imported or not: the list is recomputed only when that
  session's Stop hook runs again.
...
- **The timestamp gate is the expression itself, and every boundary it draws is
  now held by a test.** `IsoTimestampSchema` validated through a `.refine` and
  restated the same expression in `.meta`, so the runtime check and the
  published `pattern` were two copies kept in step by hand. It is a `.regex`
  now: zod emits that as the `pattern`, so there is one expression and nowhere
  for a second to drift.

  Two boundaries had been pinned before -- that seconds are optional, and that
  an offset or `Z` is required -- and the rest were implied by a few examples.
  Admitting lowercase designators, or dropping the pattern from the artifact
  entirely, passed the whole suite: the drift guard compares generated bytes
  against committed bytes, so a source edit moves both sides together and the
  guard stays green. Tests now hold each boundary by name, in the schema and
  again in the published artifact: the hour, minute and offset ranges, the days
  a month actually has including the century and 400-year rules, the leading
  anchor, and that a time of day is required at all.

  The published bytes move in key order and in the description, which now scopes
  its claim to ECMA-262 pattern semantics. **The accepted set is unchanged**, so
  no `$id` and no `schema_version` moves.
...
- **A session label can no longer break the generated documents.** A label is
  ...
  Rendered raw it broke the document twice: the newline ended the table row or
  the bullet early, and a prompt line beginning with `#` became a heading of the
  document itself. Measured on one store, 206 lines of the handoff session table
  had turned into headings.

  Orientation had the same defect on its latest-session and recent-direction
  lines. It collapsed whitespace for a note and for a track's rationale, but
  never for a label -- and orientation is the document a SessionStart hook
  injects, so those stray headings landed inside what an agent reads. Both
  documents now collapse a label before rendering it, and the handoff table
